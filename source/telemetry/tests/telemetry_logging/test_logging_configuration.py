...
def test_configure_logging__then_environmental_variables_are_set(
    mock_logging_settings, mock_logging_extras
):
    """
    ...
    # Act
    configure_logging(
        logging_settings=mock_logging_settings, extras=mock_logging_extras
    )
...
def test_get_extras__when_no_extras_none_are_returned(mock_logging_settings):
    ...
    expected_extras = {
        "orchestration_instance_id": str(
            mock_logging_settings.orchestration_instance_id
        )
    }

    configure_logging(
        logging_settings=mock_logging_settings, extras=initial_extras
    )
...
def test_get_extras__when_set_extras_are_returned(mock_logging_settings):
    ...
    expected_extras["orchestration_instance_id"] = str(
        mock_logging_settings.orchestration_instance_id
    )
    configure_logging(
        logging_settings=mock_logging_settings, extras=initial_extras
...
def test_configure_logging__when_connection_string_is_provided__azure_monitor_is_configured(
    ...
    mock_logging_settings.applicationinsights_connection_string = (
        connection_string
    )
...
def test_configure_logging_check_if_logging_configured(
    mock_logging_settings, mock_logging_extras
):
    # Arrange
...
def test_logging_settings_from_mock(mock_env_args, mock_sys_args):
    ...
    expected_applicationinsights_connection_string = (
    ...
    with (
        ...
        assert (
            ...
            == expected_orchestration_instance_id
        )
        assert (
            logging_settings.force_configuration
            is expected_force_configuration
        )
...
def test_logging_settings_without_cli_params(mock_env_args):
    ...
    expected_applicationinsights_connection_string = (
        "applicationinsights_connection_string_value"  # From Mock
    )
...
def test_logging_settings_params_from_both_cli_and_env(
    mock_env_args, mock_sys_args
):
