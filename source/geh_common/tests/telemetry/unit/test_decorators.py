...
def test_start_trace__when_unhandled_exception__exits_with_code_4(
    ...
    log it (with traceback) and exit with code 4. This test uses the real Logger
    to catch missing methods (e.g. Logger.exception not existing)."""
    import logging
    ...
    assert any(
        "Unhandled exception in app_sample_function" in r.message and r.exc_info is not None for r in caplog.records
    )
