...
from tests.testing.unit.scenario_testing.constants import SCENARIO_TESTING_TEST_CASE
...
def test_write_for_single_when_file(spark):
    ...
    write_to_delta.write_when_files_to_delta(spark=spark, scenario_path=str(SCENARIO_TESTING_TEST_CASE), files=files)
...
def test_file_is_ignored_if_it_does_not_exist_in_when(spark):
    ...
    is_table_created_for_file_name1 = (
        tables_df.filter(tables_df.tableName == file_name1.removesuffix(".csv")).count() > 0
    )
    is_table_created_for_file_name2 = (
        tables_df.filter(tables_df.tableName == file_name2.removesuffix(".csv")).count() > 0
    )
