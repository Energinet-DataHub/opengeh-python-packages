...
def test_missing_function(caplog):
    with caplog.at_level(logging.ERROR):
        with pytest.raises(AssertionError, match="Missing script entry points detected"):
            assert_pyproject_toml_project_script_exists(test_cases / "case_missing" / "pyproject.toml.test")
...
def test_success(caplog):
    with caplog.at_level(logging.INFO):
        assert_pyproject_toml_project_script_exists(test_cases / "case_succeed" / "pyproject.toml.test")
