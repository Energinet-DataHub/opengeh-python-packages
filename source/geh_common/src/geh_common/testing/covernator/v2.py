...
cases_df["scenarios"] = cases_df["case"].map(lambda x: case_to_scenarios.get(x, []))
...
for _, row in filtered_scenarios.iterrows():
    with st.expander(f"{row['group']} - {row['path']}"):
        st.write("### Cases:")
...
if path_filter:
    filtered_cases = filtered_cases[filtered_cases["path"].str.contains(path_filter, case=False, na=False)]

st.subheader("Cases")
for _, row in filtered_cases.iterrows():
    with st.expander(
        f"{row['group']} - {row['path']} - {row['case']} ({'Implemented' if row['implemented'] else 'Not Implemented'})"
    ):
        st.write("### Scenarios:")
