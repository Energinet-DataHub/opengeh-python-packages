...
from testcommon.dataframes import write_to_delta, assert_dataframes_and_schemas
from testcommon.scenario_testing import get_then_names

from tests.scenario_testing.constants import SCENARIO_TESTING_TEST_CASE
...
def test_write_to_delta_creates_table_with_same_name_as_file(spark):
    ...
    has_created_table = tables_df.filter(tables_df.tableName == file_name.removesuffix('.csv')).count() > 0
...
def test_write_to_delta_overwrites_previous_data(spark):
    ...
    # Insert dummy data
    dummy_df = spark.createDataFrame([('string1', 'string2', 'string3')], schema_for_input)
    dummy_df.write.mode("overwrite").format("delta").saveAsTable(file_name.removesuffix(".csv"))
...
def test_file_is_ignored_if_it_does_not_exist_in_when(spark):
    ...
    # Checks
    tables_df = spark.sql("SHOW TABLES")
    is_table_created_for_file_name1 = tables_df.filter(
        tables_df.tableName == file_name1.removesuffix('.csv')).count() > 0
    is_table_created_for_file_name2 = tables_df.filter(
        tables_df.tableName == file_name2.removesuffix('.csv')).count() > 0
