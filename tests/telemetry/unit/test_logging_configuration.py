...
def mock_env_args(request):
    env_args = {"APPLICATIONINSIGHTS_CONNECTION_STRING": "applicationinsights_connection_string_value"}
    return env_args
...
def mock_logging_settings():
    ...
    with mock.patch("sys.argv", sys_args), mock.patch.dict("os.environ", env_args):
...
def test_get_extras__when_no_extras_none_are_returned(mock_logging_settings):
    ...
    expected_extras = {
        "orchestration_instance_id": str(mock_logging_settings.orchestration_instance_id),
        "subsystem": str(mock_logging_settings.subsystem),
    }
...
def test_add_extras__extras_can_be_added_and_initial_extras_are_kept(mock_logging_settings, mock_logging_extras):
    ...
    orch_instance_id_and_subsystem = {
        "orchestration_instance_id": str(mock_logging_settings.orchestration_instance_id),
        "subsystem": str(mock_logging_settings.subsystem),
    }
    new_extras = {"new_key": "new_value"}
    expected_extras = orch_instance_id_and_subsystem | initial_extras | new_extras
